...
static void
lab5_prog_1(struct svc_req *rqstp, SVCXPRT *transp)
{
	union {
...
main( int argc, char* argv[] )
{
	...
	if (getsockname(0, (struct sockaddr *)&saddr, &asize) == 0) {
		...
		_rpcpmstart = 1;
		proto = 0;
		openlog("lab5", LOG_PID, LOG_DAEMON);
	} else {
#ifndef RPC_SVC_FG
		int size;
		int pid, i;
...
#endif
		sock = RPC_ANYSOCK;
		(void) pmap_unset(LAB5_PROG, LAB5_VERS);
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, LAB5_PROG, LAB5_VERS, lab5_prog_1, proto)) {
			_msgout("unable to register (LAB5_PROG, LAB5_VERS, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, LAB5_PROG, LAB5_VERS, lab5_prog_1, proto)) {
			_msgout("unable to register (LAB5_PROG, LAB5_VERS, tcp).");
			exit(1);
		}
	}
